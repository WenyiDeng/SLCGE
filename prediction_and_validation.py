...
DTI = np.loadtxt("dataset/mat_drug_protein.txt")#标签矩阵
index1 = np.loadtxt('result1/index_1.txt')
index0 = np.loadtxt('result1/index_0.txt')
...
for f in tqdm(range(5)):
    pre = np.loadtxt('result/fold_'+str(f)+'_pre.txt')
    idx = index[f, :]
...
# 对score排序
TOPK=30
drug_id=np.loadtxt('dataset/drug.txt',dtype=str)
protein_id=np.loadtxt('dataset/protein.txt',dtype=str)
...
drug_dict_path='dataset/drug_dict_map.txt'
target_dict_path='dataset/protein_dict_map.txt'
