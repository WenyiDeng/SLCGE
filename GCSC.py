...
A = np.loadtxt('result/A'+str(fold)+'.txt')#其中A有WKNKN的信息
X = np.loadtxt('result/X'+str(fold)+'.txt')#其中X有融合相似度的信息
...
if adjust_p_neighbors_parameters==True:
    ...
    # np.savetxt('dataset/SD_708_708_after_sparse.txt', Sd_after_sparse)
    # np.savetxt('dataset/ST_1512_1512_after_sparse.txt', St_after_sparse)
    ...
    Sd_after_sparse=np.loadtxt('dataset/drug_similarity_max_708_708_after_sparse.txt')
    St_after_sparse=np.loadtxt('dataset/target_similarity_max_1512_1512_after_sparse.txt')
...
np.savetxt('result_GCSC/score'+str(fold)+'.txt', result)
np.savetxt('result_GCSC/embedding'+str(fold)+'.txt', embedding)
