...
# SD = np.loadtxt("dataset/SD_708_708.txt")
SD = np.loadtxt('dataset/Similarity_Matrix_Drugs.txt')
# ST = np.loadtxt("dataset/ST_1512_1512.txt")
ST = np.loadtxt('dataset/Similarity_Matrix_Proteins.txt')
DDI = np.loadtxt("dataset/mat_drug_drug.txt")
TTI = np.loadtxt("dataset/mat_protein_protein.txt")
A = np.loadtxt("dataset/mat_drug_protein.txt")   # A矩阵  原始标签数据
A_DTI=np.loadtxt('dataset/DTI_708_1512_MAX_DISCRETIZE.txt')
...
np.savetxt("result1/index_1.txt", grouped_data1)      # 5*384
...
np.savetxt("result1/index_0.txt", grouped_data0)      # 5*213714
...
while f < fold:
    ...
    np.savetxt("result/X"+str(f)+".txt", X)
    ...
    np.savetxt("result/A" + str(f) + ".txt", adj)
    f += 1